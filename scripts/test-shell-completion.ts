 ...
 * `hands update`. Use it to verify the completion installation flow
...
\`hands update\`, \`hands doctor\`, and \`hands onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Hands Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`hands completion --install\` later to enable.`));
    return;
  }
