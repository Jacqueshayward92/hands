...
This is an **alpha** iOS app that connects to an Hands Gateway as a `role: node`.
...
3. On iOS: Hands → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project Hands.xcodeproj -scheme Hands -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/HandsKit` contains the shared transport/types used by the iOS app.
