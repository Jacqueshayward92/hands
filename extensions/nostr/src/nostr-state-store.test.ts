import type { PluginRuntime } from "hands/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.HANDS_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-nostr-"));
  process.env.HANDS_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.HANDS_STATE_DIR?.trim() || env.HANDS_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".hands");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.HANDS_STATE_DIR = previous;
    }
