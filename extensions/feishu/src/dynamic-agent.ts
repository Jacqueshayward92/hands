import type { HandsConfig, PluginRuntime } from "hands/plugin-sdk";
...
export async function maybeCreateDynamicAgent(params: {
  ...
  const workspaceTemplate = dynamicCfg.workspaceTemplate ?? "~/.hands/workspace-{agentId}";
  const agentDirTemplate = dynamicCfg.agentDirTemplate ?? "~/.hands/agents/{agentId}/agent";
