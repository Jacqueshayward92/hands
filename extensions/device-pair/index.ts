...
import { approveDevicePairing, listDevicePairing } from "hands/plugin-sdk";
...
function resolveGatewayPort(cfg: HandsPluginApi["config"]): number {
  const envRaw =
    process.env.HANDS_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: HandsPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: HandsPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: HandsPluginApi): Promise<ResolveUrlResult> {
