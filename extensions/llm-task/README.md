...
`hands.invoke --each`) without adding new Hands code per workflow.
...
This extension depends on Hands internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Hands extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.hands/extensions` as a standalone plugin directory.
