...
import type { HandsPluginApi, HandsPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "hands-lobster-plugin-") {
...
function fakeApi(overrides: Partial<HandsPluginApi> = {}): HandsPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<HandsPluginToolContext> = {}): HandsPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: HandsPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
