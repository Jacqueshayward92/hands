...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as HandsConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as HandsConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as HandsConfig);
