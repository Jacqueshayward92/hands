...
metadata: { "hands": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Hands `sessions_spawn`
- **File I/O** == Hands `read`/`write`
- **Remote fetch** == Hands `web_fetch` (or `exec` with curl when POST is required)
