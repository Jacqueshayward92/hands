/**
 * Twitch channel plugin for Hands.
 *
...
import { buildChannelConfigSchema } from "hands/plugin-sdk";
import type {
 ...
 * for Hands. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: HandsConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: HandsConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: HandsConfig): boolean => {
