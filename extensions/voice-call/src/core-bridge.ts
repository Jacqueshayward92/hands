...
function resolveHandsRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.HANDS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set HANDS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveHandsRoot(), "dist", "extensionAPI.js");
