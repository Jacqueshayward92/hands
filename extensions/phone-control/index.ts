import type { HandsPluginApi, HandsPluginService } from "hands/plugin-sdk";
...
function normalizeDenyList(cfg: HandsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: HandsPluginApi["config"]): string[] {
...
export default function register(api: HandsPluginApi) {
  ...
  const timerService: HandsPluginService = {
