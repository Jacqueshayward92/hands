---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Hands deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Hands cron scheduling for periodic checks, or version status checks on a machine running Hands (laptop, workstation, Pi, VPS).
---

# Hands Host Hardening

## Overview

Assess and harden the host running Hands, then align it to a user-defined risk tolerance without breaking access. Use Hands security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Hands changes the host firewall, SSH, or OS updates; it does not.
...
5. Hands gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Hands can access sensitive data.
...
### 2) Run Hands security audits (read-only)

As part of the default read-only checks, run `hands security audit --deep`. Only offer alternatives if the user requests them:

1. `hands security audit` (faster, non-probing)
2. `hands security audit --json` (structured output)

Offer to apply Hands safe defaults (numbered):

1. `hands security audit --fix`

Be explicit that `--fix` only tightens Hands defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Hands version/update status (read-only)

As part of the default read-only checks, run `hands update status`.
...
- Credential hygiene notes (location of Hands creds, prefer disk encryption)
...
After Hands install or first hardening pass, run at least one baseline audit and version check:

- `hands security audit`
- `hands security audit --deep`
- `hands update status`

Ongoing monitoring is recommended. Use the Hands cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `hands cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `hands update status`
...
Before creating, `hands cron list` and match on exact `name`. If found, `hands cron edit <id> ...`.
If not found, `hands cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `hands update status` (preferred for source checkouts and channels)
2. `npm view hands version` (published npm version)

## Hands command accuracy

Use only supported commands and flags:

- `hands security audit [--deep] [--fix] [--json]`
- `hands status` / `hands status --deep`
- `hands health --json`
- `hands update status`
- `hands cron add|list|runs|run`

Do not invent CLI flags or imply Hands enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Hands compaction:
