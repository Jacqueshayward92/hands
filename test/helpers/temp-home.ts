...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("HANDS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.HANDS_STATE_DIR = path.join(base, ".hands");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "hands-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".hands", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
