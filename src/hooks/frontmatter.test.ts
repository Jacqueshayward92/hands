...
`;
    ...
    expect(parsed.hands.emoji).toBe("💾");
    expect(parsed.hands.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.hands.emoji).toBe("📝");
    expect(parsed.hands.events).toEqual(["command"]);
    expect(parsed.hands.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.hands.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"hands": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"hands": {"events": ["test"]}}');
  });
...
describe("resolveHandsMetadata", () => {
  it("extracts hands metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveHandsMetadata(frontmatter);
  ...
  it("returns undefined when hands key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        hands: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Hands" },
            { id: "npm", kind: "npm", package: "@hands/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@hands/hook");
  });
...
`;
    ...
    const hands = resolveHandsMetadata(frontmatter);
    expect(hands).toBeDefined();
    expect(hands?.emoji).toBe("💾");
    expect(hands?.events).toEqual(["command:new"]);
    expect(hands?.requires?.config).toEqual(["workspace.dir"]);
    expect(hands?.install?.[0].kind).toBe("bundled");
  });
