...
homepage: https://docs.hands.ai/hooks#command-logger
metadata:
  {
    "hands":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.hands/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.hands/logs/commands.log ~/.hands/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/hands`:
   ```
   /home/username/.hands/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
