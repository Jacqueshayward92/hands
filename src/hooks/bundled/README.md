...
This directory contains hooks that ship with Hands. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.hands/hands.json`:
...
2. Restart gateway: `pkill -9 -f 'hands.*gateway' && pnpm hands gateway`
3. Enable the hook: `hands hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.hands.ai/hooks
