...
declare const __HANDS_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __HANDS_VERSION__ === "string" && __HANDS_VERSION__) ||
  process.env.HANDS_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `hands --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.HANDS_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.HANDS_GATEWAY_BIND ??
