...
const requireAgentDir = () => {
  const agentDir = process.env.HANDS_AGENT_DIR;
  if (!agentDir) {
    throw new Error("HANDS_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.HANDS_STATE_DIR;
  const previousAgentDir = process.env.HANDS_AGENT_DIR;
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) {
      delete process.env.HANDS_AGENT_DIR;
    } else {
      process.env.HANDS_AGENT_DIR = previousAgentDir;
    }
    ...
    delete process.env.HANDS_OAUTH_DIR;
  });

  it("writes auth-profiles.json under HANDS_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-oauth-"));
    process.env.HANDS_STATE_DIR = tempStateDir;
    process.env.HANDS_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.HANDS_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to HANDS_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-minimax-"));
    process.env.HANDS_STATE_DIR = tempStateDir;
    process.env.HANDS_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.HANDS_AGENT_DIR;
