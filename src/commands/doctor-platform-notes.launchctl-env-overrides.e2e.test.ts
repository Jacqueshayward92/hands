...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "HANDS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("HANDS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv HANDS_GATEWAY_TOKEN");
    expect(message).not.toContain("HANDS_GATEWAY_PASSWORD");
  });
