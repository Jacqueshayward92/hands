...
beforeAll(() => {
  previousProfile = process.env.HANDS_PROFILE;
  process.env.HANDS_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.HANDS_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/hands-root.js", () => ({
  resolveHandsPackageRoot: vi.fn().mockResolvedValue("/tmp/hands"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/hands/pnpm-lock.yaml",
      markerPath: "/tmp/hands/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("Hands status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("hands status --all") ||
          l.includes("hands --profile isolated status --all") ||
          l.includes("hands status --all") ||
          l.includes("hands --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.HANDS_GATEWAY_TOKEN = "abcd1234";
    try {
