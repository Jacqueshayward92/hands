...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveHandsAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("hands models auth setup-token")}\` or \`${formatCliCommand("hands configure")}\`.`
          : `Run \`${formatCliCommand("hands configure")}\` or set an API key env var.`;
