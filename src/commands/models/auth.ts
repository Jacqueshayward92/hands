...
import { readConfigFileSnapshot, type HandsConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: HandsConfig, model: string): HandsConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("hands plugins install")}\`.`,
    );
  }
