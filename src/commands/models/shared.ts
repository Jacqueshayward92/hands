...
export async function updateConfig(
  mutator: (cfg: HandsConfig) => HandsConfig,
): Promise<HandsConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: HandsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: HandsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("hands agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
