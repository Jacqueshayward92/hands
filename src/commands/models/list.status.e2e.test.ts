...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveHandsAgentDir: vi.fn().mockReturnValue("/tmp/hands-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/hands-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/hands-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveHandsAgentDir: mocks.resolveHandsAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveHandsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/hands-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/hands-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/hands-agent-custom");
