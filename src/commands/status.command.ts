...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Hands status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("hands security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("hands security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.hands.ai/faq");
  runtime.log("Troubleshooting: https://docs.hands.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("hands status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("hands logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("hands status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("hands gateway probe")}`);
  }
}
