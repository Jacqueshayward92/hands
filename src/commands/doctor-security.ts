...
import type { HandsConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: HandsConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("hands security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("hands configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("hands config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("hands doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("hands config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('hands config set session.dmScope "per-channel-peer"') +
