...
beforeEach(() => {
  ...
  resolveHandsPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.HANDS_STATE_DIR;
  originalUpdateInProgress = process.env.HANDS_UPDATE_IN_PROGRESS;
  process.env.HANDS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "hands-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.HANDS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.HANDS_UPDATE_IN_PROGRESS;
  } else {
    process.env.HANDS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveHandsPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadHandsPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/hands-root.js", () => ({
  resolveHandsPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "hands-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.HANDS_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when HANDS_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/hands.json",
      exists: true,
      raw: "{}",
    ...
    process.env.HANDS_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
