...
export function listAgentEntries(cfg: HandsConfig): AgentEntry[] {
...
function resolveAgentName(cfg: HandsConfig, agentId: string) {
...
function resolveAgentModel(cfg: HandsConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: HandsConfig): AgentSummary[] {
