...
const SEARCH_TOOL = "https://docs.hands.ai/mcp.SearchHands";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.hands.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('hands docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.hands.ai/");
      runtime.log(`Search: ${formatCliCommand('hands docs "your query"')}`);
    }
    return;
  }
