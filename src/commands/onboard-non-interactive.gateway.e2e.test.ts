...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "hands-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.HANDS_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "hands");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.HANDS_STATE_DIR = stateDir;
    process.env.HANDS_CONFIG_PATH = path.join(stateDir, "hands.json");
