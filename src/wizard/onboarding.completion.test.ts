...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "hands",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/hands.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("hands");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "hands");
