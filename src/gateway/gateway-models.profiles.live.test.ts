...
import type { HandsConfig, ModelProviderConfig } from "../config/types.js";
import { resolveHandsAgentDir } from "../agents/agent-paths.js";
...
import { ensureHandsModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.HANDS_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.HANDS_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.HANDS_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<HandsConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.HANDS_AGENT_DIR,
  ...
  const hostAgentDir = resolveHandsAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-live-state-"));
  process.env.HANDS_STATE_DIR = tempStateDir;
  ...
  process.env.HANDS_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.hands-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveHandsAgentDir();
  const sanitizedCfg: HandsConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Hands live tool probe (local, safe): " +
    ...
    process.env.HANDS_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.HANDS_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureHandsModelsJson(cfg);
      ...
      const rawModels = process.env.HANDS_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.hands-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
