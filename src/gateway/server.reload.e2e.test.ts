...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.HANDS_SKIP_CHANNELS;
    prevSkipGmail = process.env.HANDS_SKIP_GMAIL_WATCHER;
    process.env.HANDS_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.HANDS_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.HANDS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.HANDS_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
