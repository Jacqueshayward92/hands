...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.HANDS_STATE_DIR;
  previousConfigPath = process.env.HANDS_CONFIG_PATH;
  previousSkipBrowserControl = process.env.HANDS_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.HANDS_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.HANDS_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.HANDS_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "hands-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.HANDS_STATE_DIR = path.join(tempHome, ".hands");
  delete process.env.HANDS_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.HANDS_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.HANDS_SKIP_GMAIL_WATCHER = "1";
  process.env.HANDS_SKIP_CANVAS_HOST = "1";
  process.env.HANDS_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "hands-test-no-bundled-extensions")
    : "hands-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "hands-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".hands-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.HANDS_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.HANDS_CONFIG_PATH;
    } else {
      process.env.HANDS_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.HANDS_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.HANDS_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.HANDS_SKIP_GMAIL_WATCHER;
    } else {
      process.env.HANDS_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.HANDS_SKIP_CANVAS_HOST;
    } else {
      process.env.HANDS_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.HANDS_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.HANDS_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
  }
...
export async function startServerWithClient(
  ...
  const prev = process.env.HANDS_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.HANDS_GATEWAY_TOKEN = fallbackToken;
  }
