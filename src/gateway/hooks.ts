...
export function resolveHooksConfig(cfg: HandsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: HandsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-hands-token"] === "string"
      ? req.headers["x-hands-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
