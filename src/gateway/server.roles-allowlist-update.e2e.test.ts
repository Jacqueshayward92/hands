...
const connectNodeClient = async (params: {
  ...
  const token = process.env.HANDS_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("HANDS_GATEWAY_TOKEN is required for node test clients");
  }
  let settled = false;
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".hands", "restart-sentinel.json");
