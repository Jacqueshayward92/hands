...
function candidateBinDirs(opts: EnsureHandsPathOpts): string[] {
  ...
  // Bundled macOS app: `hands` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "hands");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/hands` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "hands"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `hands` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureHandsCliOnPath(opts: EnsureHandsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.HANDS_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.HANDS_PATH_BOOTSTRAPPED = "1";
