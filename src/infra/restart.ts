...
export function triggerHandsRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.HANDS_SYSTEMD_UNIT,
        process.env.HANDS_PROFILE,
      );
  ...
  const label =
    process.env.HANDS_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.HANDS_PROFILE);
