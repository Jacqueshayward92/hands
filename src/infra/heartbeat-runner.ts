...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: HandsConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: HandsConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: HandsConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: HandsConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: HandsConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: HandsConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: HandsConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
