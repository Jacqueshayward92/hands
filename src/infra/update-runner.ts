...
const DEFAULT_PACKAGE_NAME = "hands";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hands-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "hands.mjs");
    ...
    const doctorStep = await runStep(
      step("hands doctor", doctorArgv, gitRoot, { HANDS_UPDATE_IN_PROGRESS: "1" }),
    );
