...
describe("resolveEffectiveHomeDir", () => {
  it("prefers HANDS_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands HANDS_HOME when set to ~", () => {
    const env = {
      HANDS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for HANDS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { HANDS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when HANDS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ HANDS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { HANDS_HOME: "/srv/hands-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/hands-home")}/x`);
  });
