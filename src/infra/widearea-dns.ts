...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.HANDS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*hands-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "hands";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "hands");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "hands-gw");
  ...
  records.push(`_hands-gw._tcp IN PTR ${instanceLabel}._hands-gw._tcp`);
  records.push(`${instanceLabel}._hands-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._hands-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; hands-content-hash: ${contentHash}\n${contentBody}`;
}
