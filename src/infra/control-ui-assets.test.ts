...
import { resolveHandsPackageRoot } from "./hands-root.js";
...
describe("control UI assets helpers", () => {
  it("resolves repo root from src argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hands-ui-"));
    try {
  ...
  it("resolves control-ui root for dist/gateway bundle argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hands-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "hands" }));
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    ...
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "hands" }));
      await fs.writeFile(path.join(tmp, "hands.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(tmp, "hands.mjs"))).toBe(
  ...
  it("resolves control-ui root for package entrypoint argv1", async () => {
    ...
    try {
      ...
      expect(resolveControlUiRootSync({ argv1: path.join(tmp, "hands.mjs") })).toBe(
        path.join(tmp, "dist", "control-ui"),
      );
    } finally {
      await fs.rm(tmp, { recursive: true, force: true });
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "hands");
      await fs.mkdir(binDir, { recursive: true });
      await fs.mkdir(path.join(pkgRoot, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(binDir, "hands"), "#!/usr/bin/env node\n");
      await fs.writeFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "hands" }));
      await fs.writeFile(path.join(pkgRoot, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(binDir, "hands"))).toBe(
  ...
  it("resolves via fallback when package root resolution fails but package name matches", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hands-ui-"));
    try {
      // Package named "hands" but resolveHandsPackageRoot failed for other reasons
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "hands" }));
      await fs.writeFile(path.join(tmp, "hands.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");
  ...
  it("returns null when package name does not match hands", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "hands-ui-"));
    try {
  ...
  it("resolves control-ui root when argv1 is a symlink (nvm scenario)", async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "hands" }));
      await fs.writeFile(path.join(realPkg, "hands.mjs"), "export {};\n");
      ...
      const resolvedRoot = resolveControlUiRootSync({ argv1: path.join(bin, "hands") });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    try {
      ...
      const packageRoot = await resolveHandsPackageRoot({ argv1: path.join(bin, "hands") });
  ...
  it("resolves dist index path via symlinked argv1 (async)", async () => {
    ...
    try {
      ...
      const indexPath = await resolveControlUiDistIndexPath(path.join(bin, "hands"));
