...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.HANDS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.HANDS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.HANDS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.HANDS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
