...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.HANDS_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.HANDS_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.HANDS_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[hands] ${parsed.error}`);
    process.exit(2);
  }
