...
describe("canvas host state dir defaults", () => {
  ...
  it("uses HANDS_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "hands-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Hands Canvas");
    } finally {
      await handler.close();
