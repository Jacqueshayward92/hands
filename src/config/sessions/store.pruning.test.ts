...
// Mock loadConfig so resolveMaintenanceConfig() never reads a real hands.json.
...
describe("rotateSessionFile", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-rotate-"));
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    testDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-pruning-integ-"));
    storePath = path.join(testDir, "sessions.json");
    savedCacheTtl = process.env.HANDS_SESSION_CACHE_TTL_MS;
    process.env.HANDS_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  ...
  afterEach(async () => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.HANDS_SESSION_CACHE_TTL_MS;
    } else {
      process.env.HANDS_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
