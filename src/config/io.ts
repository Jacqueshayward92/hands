...
import type { HandsConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
import { compareHandsVersions } from "./version.js";
...
function coerceConfig(value: unknown): HandsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as HandsConfig;
}
...
function stampConfigVersion(cfg: HandsConfig): HandsConfig {
...
function warnIfConfigFromFuture(cfg: HandsConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareHandsVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Hands (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: HandsConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): HandsConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as HandsConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as HandsConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: HandsConfig) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(validated.config, "", envRefMap, changedPaths) as HandsConfig)
        : validated.config;
...
// module scope. `HANDS_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.HANDS_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.HANDS_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export async function writeConfigFile(cfg: HandsConfig): Promise<void> {
  clearConfigCache();
