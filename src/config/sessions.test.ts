...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-hands",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-hands#general");
  });
  ...
  it("derives session transcripts dir from HANDS_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { HANDS_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.HANDS_STATE_DIR = "/custom/state";
    try {
