...
import type { HandsConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: HandsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: HandsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = HandsSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as HandsConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as HandsConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as HandsConfig,
  };
}
