...
const ETC_HANDS_DIR = path.join(ROOT_DIR, "etc", "hands");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "hands.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcHandsPath(...parts: string[]) {
  return path.join(ETC_HANDS_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcHandsPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "hands.json"))).toEqual({
      shared: true,
    });
  });
});
