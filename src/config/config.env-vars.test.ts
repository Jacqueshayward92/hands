...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.hands/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          HANDS_STATE_DIR: path.join(home, ".hands"),
          CLAWDBOT_STATE_DIR: undefined,
          HANDS_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          HANDS_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
