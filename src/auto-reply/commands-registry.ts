...
import type { HandsConfig } from "../config/types.js";
import type {
...
export function isCommandEnabled(cfg: HandsConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: HandsConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as HandsConfig),
...
export function getCommandDetection(_cfg?: HandsConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: HandsConfig) {
