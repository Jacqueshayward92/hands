...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        HANDS_AGENT_DIR: (home) => path.join(home, ".hands", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".hands", "agent"),
      },
      prefix: "hands-rawbody-",
    },
  );
}
...
describe("RawBody directive parsing", () => {
  ...
  it("reuses non-default agent session files without throwing path validation errors", async () => {
    await withTempHome(async (home) => {
      ...
      const sessionsDir = path.join(home, ".hands", "agents", agentId, "sessions");
