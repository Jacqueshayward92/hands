...
import { scheduleGatewaySigusr1Restart, triggerHandsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Hands in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerHandsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Hands via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
