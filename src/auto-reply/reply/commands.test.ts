...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-commands-"));
...
function buildParams(commandBody: string, cfg: HandsConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
