...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "hands-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "hands-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "hands-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "hands-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.hands when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "hands-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers HANDS_HOME over HOME", () => {
    vi.stubEnv("HANDS_HOME", "/srv/hands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/hands-home"));
...
describe("shortenHomePath", () => {
  it("uses $HANDS_HOME prefix when HANDS_HOME is set", () => {
    vi.stubEnv("HANDS_HOME", "/srv/hands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/hands-home")}/.hands/hands.json`)).toBe(
      "$HANDS_HOME/.hands/hands.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $HANDS_HOME replacement when HANDS_HOME is set", () => {
    vi.stubEnv("HANDS_HOME", "/srv/hands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/hands-home")}/.hands/hands.json`),
    ).toBe("config: $HANDS_HOME/.hands/hands.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/hands")).toBe(path.resolve(os.homedir(), "hands"));
  });
  ...
  it("prefers HANDS_HOME for tilde expansion", () => {
    vi.stubEnv("HANDS_HOME", "/srv/hands-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/hands")).toBe(path.resolve("/srv/hands-home", "hands"));
