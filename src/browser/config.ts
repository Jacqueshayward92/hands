import type { BrowserConfig, BrowserProfileConfig, HandsConfig } from "../config/config.js";
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "hands" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_HANDS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "hands" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_HANDS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_HANDS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Hands browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for hands/hands CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: HandsConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_HANDS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "hands";
