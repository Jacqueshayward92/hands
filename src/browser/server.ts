...
function isAuthorizedBrowserRequest(
  ...
  if (auth.password) {
    const passwordHeader = firstHeaderValue(req.headers["x-hands-password"]).trim();
...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(`hands browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
...
export async function stopBrowserControlServer(): Promise<void> {
  ...
  try {
    ...
    logServer.warn(`hands browser stop failed: ${String(err)}`);
  }
