...
function withLoopbackBrowserAuth(
  ...
  if (headers.has("authorization") || headers.has("x-hands-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.token) {
      ...
      headers.set("x-hands-password", auth.password);
    }
  } catch {
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the Hands gateway (Hands.app menubar, or \`${formatCliCommand("hands gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the Hands browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the Hands browser control service. ${hint} (${msg})`);
}
