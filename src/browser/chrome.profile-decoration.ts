...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".hands-profile-decorated");
}
 ...
 */
export function decorateHandsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_HANDS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_HANDS_BROWSER_COLOR).toUpperCase();
