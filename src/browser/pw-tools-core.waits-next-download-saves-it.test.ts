...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredHandsTmpDir: vi.fn(() => "/tmp/hands"),
}));
vi.mock("../infra/tmp-hands-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredHandsTmpDir.mockReturnValue("/tmp/hands");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    ...
    tmpDirMocks.resolvePreferredHandsTmpDir.mockReturnValue("/tmp/hands-preferred");
    currentPage = { on, off };
    ...
    const expectedDownloadsTail = `${path.join("tmp", "hands-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredHandsTmpDir).toHaveBeenCalled();
  });
