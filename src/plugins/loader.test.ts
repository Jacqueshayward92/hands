...
import { loadHandsPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.HANDS_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `hands-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "hands.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.HANDS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadHandsPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.HANDS_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadHandsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadHandsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
