...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: HandsPluginDefinition;
  register?: HandsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as HandsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as HandsPluginDefinition;
...
export function loadHandsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverHandsPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: { "hands/plugin-sdk": pluginSdkAlias },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: HandsPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as HandsPluginModule;
    } catch (err) {
