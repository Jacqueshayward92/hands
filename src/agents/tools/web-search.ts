...
type WebSearchConfig = NonNullable<HandsConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: HandsConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("hands configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.hands.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://hands.ai",
      "X-Title": "Hands Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
