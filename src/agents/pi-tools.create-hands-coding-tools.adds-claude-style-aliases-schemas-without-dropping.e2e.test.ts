...
import { __testing, createHandsCodingTools } from "./pi-tools.js";
...
describe("createHandsCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createHandsCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "hands-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "hands-outside.txt");
