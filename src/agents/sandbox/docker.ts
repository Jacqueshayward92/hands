...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "hands.sandbox=1");
  args.push("--label", `hands.sessionKey=${params.scopeKey}`);
  args.push("--label", `hands.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `hands.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("hands.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`hands sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`hands sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("hands sandbox recreate --all");
}
