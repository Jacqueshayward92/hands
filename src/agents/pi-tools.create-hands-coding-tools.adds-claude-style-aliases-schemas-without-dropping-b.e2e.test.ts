...
describe("createHandsCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createHandsCodingTools({
      config,
    ...
    const anthropicTools = createHandsCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createHandsCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createHandsCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
