...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.HANDS_GATEWAY_PORT).toBe("18789");
    expect(env.HANDS_GATEWAY_TOKEN).toBe("secret");
    expect(env.HANDS_SERVICE_MARKER).toBe("hands");
    expect(env.HANDS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.HANDS_SERVICE_VERSION).toBe("string");
    expect(env.HANDS_SYSTEMD_UNIT).toBe("hands-gateway.service");
    if (process.platform === "darwin") {
      expect(env.HANDS_LAUNCHD_LABEL).toBe("ai.hands.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", HANDS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.HANDS_SYSTEMD_UNIT).toBe("hands-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.HANDS_LAUNCHD_LABEL).toBe("ai.hands.work");
    }
  });
});
