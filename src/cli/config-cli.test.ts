...
/**
 * Test for issue #6070:
 * `hands config set` should use snapshot.parsed (raw user config) instead of
...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "hands-config-cli-"));
  const originalEnv = { ...process.env };
  try {
    // Override config path to use temp directory
    process.env.HANDS_CONFIG_PATH = path.join(home, ".hands", "hands.json");
    await fs.mkdir(path.join(home, ".hands"), { recursive: true });
    await run(home);
  } finally {
...
async function readConfigFile(home: string): Promise<Record<string, unknown>> {
  const configPath = path.join(home, ".hands", "hands.json");
