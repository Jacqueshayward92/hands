...
import { resolveHandsPackageRoot } from "../../infra/hands-root.js";
...
const HANDS_REPO_URL = "https://github.com/hands/hands.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "hands";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("hands@")) {
    return trimmed.slice("hands@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.HANDS_GIT_DIR?.trim();
  if (override) {
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", HANDS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `HANDS_GIT_DIR points at a non-git directory: ${params.dir}. Set HANDS_GIT_DIR to an empty folder or an hands checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`HANDS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "hands.mjs");
