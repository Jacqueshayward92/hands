...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    HANDS_STATE_DIR: process.env.HANDS_STATE_DIR,
    HANDS_CONFIG_PATH: process.env.HANDS_CONFIG_PATH,
    HANDS_GATEWAY_PORT: process.env.HANDS_GATEWAY_PORT,
    HANDS_PROFILE: process.env.HANDS_PROFILE,
  };

  beforeEach(() => {
    process.env.HANDS_STATE_DIR = "/tmp/hands-cli-state";
    process.env.HANDS_CONFIG_PATH = "/tmp/hands-cli-state/hands.json";
  ...
  afterEach(() => {
    if (originalEnv.HANDS_STATE_DIR !== undefined) {
      process.env.HANDS_STATE_DIR = originalEnv.HANDS_STATE_DIR;
    } else {
      delete process.env.HANDS_STATE_DIR;
    }

    if (originalEnv.HANDS_CONFIG_PATH !== undefined) {
      process.env.HANDS_CONFIG_PATH = originalEnv.HANDS_CONFIG_PATH;
    } else {
      delete process.env.HANDS_CONFIG_PATH;
    }

    if (originalEnv.HANDS_GATEWAY_PORT !== undefined) {
      process.env.HANDS_GATEWAY_PORT = originalEnv.HANDS_GATEWAY_PORT;
    } else {
      delete process.env.HANDS_GATEWAY_PORT;
    }

    if (originalEnv.HANDS_PROFILE !== undefined) {
      process.env.HANDS_PROFILE = originalEnv.HANDS_PROFILE;
    } else {
      delete process.env.HANDS_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        HANDS_PROFILE: "dev",
        HANDS_STATE_DIR: "/tmp/hands-daemon-state",
        HANDS_CONFIG_PATH: "/tmp/hands-daemon-state/hands.json",
        HANDS_GATEWAY_PORT: "19001",
      },
