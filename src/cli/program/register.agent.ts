...
${formatHelpExamples([
  ['hands agent --to +15555550123 --message "status update"', "Start a new session."],
  ['hands agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'hands agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'hands agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['hands agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'hands agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.hands.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.hands.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['hands agents set-identity --agent main --name "Hands" --emoji "🦞"', "Set name + emoji."],
  ["hands agents set-identity --agent main --avatar avatars/hands.png", "Set avatar path."],
  [
    "hands agents set-identity --workspace ~/.hands/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "hands agents set-identity --identity-file ~/.hands/workspace/IDENTITY.md --agent main",
