...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("hands node install"), formatCliCommand("hands node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("hands node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    HANDS_LOG_PREFIX: baseEnv.HANDS_LOG_PREFIX ?? "node",
