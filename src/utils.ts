...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".hands");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.HANDS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$HANDS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via HANDS_STATE_DIR.
