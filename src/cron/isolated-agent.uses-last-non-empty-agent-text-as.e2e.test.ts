...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "hands-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".hands", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<HandsConfig> = {},
): HandsConfig {
  const base: HandsConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "hands"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".hands", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
