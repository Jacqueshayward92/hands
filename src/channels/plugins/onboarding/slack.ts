...
function setSlackDmPolicy(cfg: HandsConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Hands";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Hands`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: HandsConfig, allowFrom: string[]): HandsConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
