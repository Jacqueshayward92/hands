...
function summarizeGroupPolicy(cfg: HandsConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: HandsConfig): boolean {
...
function collectModels(cfg: HandsConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: HandsConfig): Set<string> {
  const baseCfg: HandsConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: HandsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: HandsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: HandsConfig): boolean {
...
function isBrowserEnabled(cfg: HandsConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: HandsConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: HandsConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep HANDS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("hands security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer HANDS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDockerNoopFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectNodeDenyCommandPatternFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: HandsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
