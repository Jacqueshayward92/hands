...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "hands-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    HANDS_GATEWAY_TOKEN: "test-token",
    HANDS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    HANDS_WORKSPACE_DIR: join(sandbox.rootDir, "hands"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      HANDS_DOCKER_APT_PACKAGES: undefined,
      HANDS_EXTRA_MOUNTS: undefined,
      HANDS_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("HANDS_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("HANDS_EXTRA_MOUNTS=");
    expect(envFile).toContain("HANDS_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    expect(extraCompose).toContain("hands-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("hands-home:");
  });
  ...
  it("plumbs HANDS_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      HANDS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      HANDS_EXTRA_MOUNTS: "",
      HANDS_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("HANDS_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg HANDS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
