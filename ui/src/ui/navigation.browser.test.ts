...
import { HandsApp } from "./app.ts";
import "../styles.css";

// oxlint-disable-next-line typescript/unbound-method
const originalConnect = HandsApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("hands-app") as HandsApp;
...
beforeEach(() => {
  HandsApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__HANDS_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  HandsApp.prototype.connect = originalConnect;
  window.__HANDS_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/hands/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/hands");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/hands/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__HANDS_CONTROL_UI_BASE_PATH__ = "/hands";
    const app = mountApp("/hands/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/hands");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/hands/sessions");
  });
