...
export async function handleWhatsAppStart(host: HandsApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: HandsApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: HandsApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: HandsApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: HandsApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: HandsApp): string {
...
function resolveGatewayHttpAuthHeader(host: HandsApp): string | null {
...
function buildGatewayHttpHeaders(host: HandsApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: HandsApp) {
...
export function handleNostrProfileToggleAdvanced(host: HandsApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: HandsApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: HandsApp) {
  const state = host.nostrProfileFormState;
