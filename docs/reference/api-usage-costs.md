...
Hands features that can generate provider usage or paid API calls.
...
- `hands status --usage` and `hands channels list` show provider **usage windows**
...
Hands can pick up credentials from:
...
`hands models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
