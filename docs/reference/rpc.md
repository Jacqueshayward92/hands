...
Hands integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Hands owns lifecycle when `channels.signal.autoStart=true`.
...
- Hands spawns `imsg rpc` as a child process (legacy iMessage integration).
