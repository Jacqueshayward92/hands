...
你可以在 `~/.hands/hands.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/hands.log"
  }
}
```
...
```bash
hands channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.hands/hands.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/hands/hands-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Hands 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
HANDS_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `hands plugins enable diagnostics-otel` 启用插件。
...
- `hands.tokens`（计数器，属性：`hands.token`、`hands.channel`、`hands.provider`、`hands.model`）
- `hands.cost.usd`（计数器，属性：`hands.channel`、`hands.provider`、`hands.model`）
- `hands.run.duration_ms`（直方图，属性：`hands.channel`、`hands.provider`、`hands.model`）
- `hands.context.tokens`（直方图，属性：`hands.context`、`hands.channel`、`hands.provider`、`hands.model`）

消息流：

- `hands.webhook.received`（计数器，属性：`hands.channel`、`hands.webhook`）
- `hands.webhook.error`（计数器，属性：`hands.channel`、`hands.webhook`）
- `hands.webhook.duration_ms`（直方图，属性：`hands.channel`、`hands.webhook`）
- `hands.message.queued`（计数器，属性：`hands.channel`、`hands.source`）
- `hands.message.processed`（计数器，属性：`hands.channel`、`hands.outcome`）
- `hands.message.duration_ms`（直方图，属性：`hands.channel`、`hands.outcome`）

队列 + 会话：

- `hands.queue.lane.enqueue`（计数器，属性：`hands.lane`）
- `hands.queue.lane.dequeue`（计数器，属性：`hands.lane`）
- `hands.queue.depth`（直方图，属性：`hands.lane` 或 `hands.channel=heartbeat`）
- `hands.queue.wait_ms`（直方图，属性：`hands.lane`）
- `hands.session.state`（计数器，属性：`hands.state`、`hands.reason`）
- `hands.session.stuck`（计数器，属性：`hands.state`）
- `hands.session.stuck_age_ms`（直方图，属性：`hands.state`）
- `hands.run.attempt`（计数器，属性：`hands.attempt`）

### 导出的 span（名称 + 关键属性）

- `hands.model.usage`
  - `hands.channel`、`hands.provider`、`hands.model`
  - `hands.sessionKey`、`hands.sessionId`
  - `hands.tokens.*`（input/output/cache_read/cache_write/total）
- `hands.webhook.processed`
  - `hands.channel`、`hands.webhook`、`hands.chatId`
- `hands.webhook.error`
  - `hands.channel`、`hands.webhook`、`hands.chatId`、`hands.error`
- `hands.message.processed`
  - `hands.channel`、`hands.outcome`、`hands.chatId`、`hands.messageId`、`hands.sessionKey`、`hands.sessionId`、`hands.reason`
- `hands.session.stuck`
  - `hands.state`、`hands.ageMs`、`hands.queueDepth`、`hands.sessionKey`、`hands.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `hands doctor`。
