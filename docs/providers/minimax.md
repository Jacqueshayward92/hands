---
summary: "Use MiniMax M2.1 in Hands"
read_when:
  - You want MiniMax models in Hands
...
```bash
hands plugins enable minimax-portal-auth  # skip if already loaded.
hands gateway restart  # restart if gateway is already running
hands onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/hands/hands/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `hands.json`:
...
- Use `hands models list` and `hands models set minimax/MiniMax-M2.1` to switch.
...
- Running `hands configure` and selecting **MiniMax M2.1**, or
