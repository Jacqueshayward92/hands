---
summary: "Use Venice AI privacy-focused models in Hands"
read_when:
  - You want privacy-focused inference in Hands
...
```bash
hands onboard --auth-choice venice-api-key
```
...
```bash
hands chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Hands shows all available Venice models. Pick based on your needs:
...
```bash
hands models set venice/claude-opus-45
hands models set venice/llama-3.3-70b
```

List all available models:

```bash
hands models list | grep venice
```

## Configure via `hands configure`

1. Run `hands configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Hands automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
hands chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
hands chat --model venice/claude-opus-45

# Use uncensored model
hands chat --model venice/venice-uncensored

# Use vision model with image
hands chat --model venice/qwen3-vl-235b-a22b

# Use coding model
hands chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `hands models list` to see currently available models. Some models may be temporarily offline.
