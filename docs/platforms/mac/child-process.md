...
service via the external `hands` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`HANDS_PROFILE`; legacy `com.hands.*` is supported).
