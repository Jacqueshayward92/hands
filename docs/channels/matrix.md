...
Matrix is an open, decentralized messaging protocol. Hands connects as a Matrix **user**
...
```bash
hands plugins install @hands/matrix
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `hands plugins install @hands/matrix`
   - From a local checkout: `hands plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Hands calls the same
     login endpoint, stores the access token in `~/.hands/credentials/matrix/credentials.json`,
...
- On first connection, Hands requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Hands logs a warning.
...
`~/.hands/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `hands pairing list matrix`
  - `hands pairing approve matrix <CODE>`
...
- On startup, Hands resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
