...
```
    ...
    - or read `chat.id` from `hands logs --follow`
...
- DM messages can carry `message_thread_id`; Hands routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    Hands can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Hands retries as plain text.
...
```
    ...
    - `~/.hands/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Hands enqueues system events like:
...
```bash
hands message send --channel telegram --target 123456789 --message "hi"
hands message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `hands channels status` warns when config expects unmentioned group messages.
    - `hands channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `hands logs --follow` for skip reasons
