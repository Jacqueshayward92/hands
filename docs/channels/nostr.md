...
read_when:
  - You want Hands to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Hands to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`hands onboard`) and `hands channels add` list optional channel plugins.
...
```bash
hands plugins install @hands/nostr
```

Use a local checkout (dev workflows):

```bash
hands plugins install --link <path-to-hands>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "hands@example.com",
        "lud16": "hands@example.com"
      }
    }
  }
}
