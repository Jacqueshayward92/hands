...
read_when:
  - Setting up Twitch chat integration for Hands
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Hands connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
hands plugins install @hands/twitch
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `HANDS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "hands", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use HANDS_TWITCH_ACCESS_TOKEN env var)
...
```bash
HANDS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
