...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Hands to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
hands plugins install @hands/feishu
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/feishu
```
...
If you just installed Hands, run the wizard:
...
2. Configuring app credentials in Hands
...
1. You already ran `hands channels add` for Feishu
2. The gateway is running (`hands gateway status`)
...
```bash
hands pairing approve feishu <CODE>
```
...
2. Run `hands logs --follow` and look for `chat_id`
...
2. Run `hands logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `hands gateway status`  | Show gateway status           |
| `hands gateway install` | Install/start gateway service |
| `hands gateway stop`    | Stop gateway service          |
| `hands gateway restart` | Restart gateway service       |
| `hands logs --follow`   | Tail gateway logs             |
...
4. Check logs: `hands logs --follow`
...
5. Ensure the gateway is running: `hands gateway status`
6. Check logs: `hands logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.hands/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.hands/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
