...
```bash
hands plugins install @hands/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Hands" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `hands pairing list nextcloud-talk`
  - `hands pairing approve nextcloud-talk <CODE>`
