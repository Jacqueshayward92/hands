...
Tlon is a decentralized messenger built on Urbit. Hands connects to your Urbit ship and can
...
```bash
hands plugins install @hands/tlon
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/tlon
```
...
Use these with `hands message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Hands replies in-thread.
