...
```bash
hands plugins install @hands/msteams
```

Local checkout (when running from a git repo):

```bash
hands plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Hands will offer the local install path automatically.
...
3. Configure Hands with those credentials.
...
- Talk to Hands via Teams DMs, group chats, or channels.
...
- On startup, Hands resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.hands/hands.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Hands, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `hands-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `hands plugins install @hands/msteams`
   - From a local checkout: `hands plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Hands in Teams", "full": "Hands in Teams" },
...
Hands handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Hands only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/HandsShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Hands sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `hands message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.hands/msteams-polls.json`.
...
```bash
hands message send --channel msteams \
...
hands message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
hands message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
hands message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
hands message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
