...
- Jobs persist under `~/.hands/cron/` so restarts don’t lose schedules.
...
hands cron list
hands cron run <job-id>
hands cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.hands/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `hands cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, Hands defaults to `announce`.
...
- Job store: `~/.hands/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.hands/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
hands cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
hands cron edit <jobId> --agent ops
hands cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
hands cron run <jobId>
hands cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
hands cron edit <jobId> \
...
```bash
hands system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`hands system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `HANDS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Hands applies exponential retry backoff for recurring jobs after consecutive errors:
