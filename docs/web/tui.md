...
```bash
hands tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `hands tui --deliver`
...
- Check the Gateway logs: `hands logs --follow`.
- Confirm the agent can run: `hands status` and `hands models status`.
...
- No agents in picker: check `hands agents list` and your routing config.
