...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hands`)
...
If the page fails to load, start the Gateway first: `hands gateway`.
...
you revoke it with `hands devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.hands/hands.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Hands
...
Hands **blocks** Control UI connections without device identity.
