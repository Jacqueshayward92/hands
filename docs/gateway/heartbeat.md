...
- During heartbeat runs, Hands treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `hands sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Hands skips the heartbeat run entirely (no model call).
...
```bash
hands system event --text "Check for urgent follow-ups" --mode now
```
