...
description: "Complete field-by-field reference for ~/.hands/hands.json"
---

# Configuration Reference

Every field available in `~/.hands/hands.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Hands uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `hands doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `hands plugins install @hands/mattermost`.
...
Hands spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@hands"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `hands.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Hands auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Hands assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.hands/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.hands/workspace",
        agentDir: "~/.hands/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@hands"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:hands`   | All built-in tools (excludes provider plugins)                                           |
...
Hands uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.hands/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `HANDS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `hands onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `hands onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `hands onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `hands onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `hands onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `hands onboard --auth-choice minimax-api`.
...
- Loaded from `~/.hands/extensions`, `<workspace>/.hands/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or HANDS_GATEWAY_PASSWORD
      allowTailscale: true,
      rateLimit: {
        maxAttempts: 10,
        windowMs: 60000,
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `HANDS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.hands-dev` + port `19001`), `--profile <name>` (uses `~/.hands-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-hands-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `HANDS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or HANDS_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__hands__/a2ui/`.
...
- Hostname defaults to `hands`. Override with `HANDS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.hands/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `hands dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.hands/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${HANDS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.hands/credentials/oauth.json`.
...
- Default log file: `/tmp/hands/hands-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `hands doctor --fix` can strip unknown keys).
