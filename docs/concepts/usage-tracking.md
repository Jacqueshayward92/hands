...
- `/usage cost` in chats: local cost summary aggregated from Hands session logs.
- CLI: `hands status --usage` prints a full per-provider breakdown.
- CLI: `hands channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
