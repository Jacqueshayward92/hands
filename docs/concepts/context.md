...
read_when:
  - You want to understand what “context” means in Hands
...
“Context” is **everything Hands sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Hands-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Hands builds the system prompt

The system prompt is **Hands-owned** and rebuilt each run. It includes:
...
By default, Hands injects a fixed set of workspace files (if present):
