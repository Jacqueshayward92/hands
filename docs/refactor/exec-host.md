...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.hands/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.hands/node.json` (node id + pairing token).
...
Path: `~/.hands/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.hands/exec-approvals.sock",
...
- Unix socket at `~/.hands/exec-approvals.sock` (0600).
