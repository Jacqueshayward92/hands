...
- Publish as `hands/plugin-sdk` (or export from core under `hands/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `HandsPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: HandsConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: HandsConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: HandsConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: HandsConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: HandsConfig): string;
  };
};
```
...
- Introduce `hands/plugin-sdk`.
- Add `api.runtime` to `HandsPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `handsRuntime: ">=2026.2.0"`).
