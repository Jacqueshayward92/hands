...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Hands
...
Running Hands via Node with `tsx` fails at startup with:

```
[hands] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hands status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch hands.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node hands.mjs status` works on Node 25.
