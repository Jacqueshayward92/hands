...
read_when:
  - Designing workspace memory (~/.hands/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Hands integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.hands/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.hands/workspace` as canonical human-readable memory.
...
```
~/.hands/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Hands**, but keep a separable core library.

### Why integrate into Hands?
...
- You want the agent itself to call the tools:
  - `hands memory recall "…" --k 25 --since 30d`
  - `hands memory reflect --since 7d`
...
Pragmatic take for `~/.hands/workspace`:
