...
Hands can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Hands can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Hands defaults
...
TTS config lives under `messages.tts` in `hands.json`.
...
- If `provider` is **unset**, Hands prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.hands/settings/tts.json`, override with `HANDS_TTS_PREFS` or
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Edge output format fails, Hands retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Hands registers
