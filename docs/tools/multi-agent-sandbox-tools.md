...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `hands sandbox explain`.
...
Legacy `agent.*` configs are migrated by `hands doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=hands-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${HANDS_STATE_DIR:-$HOME/.hands}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
