...
- **gateway host** → `hands` process on the gateway machine
...
`~/.hands/exec-approvals.json` directly.

CLI: `hands approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
