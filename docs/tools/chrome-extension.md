---
summary: "Chrome extension: let Hands drive your existing Chrome tab"
read_when:
...
The Hands Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate hands-managed Chrome profile.
...
Hands then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Hands release (npm package) as static files. There is no separate “build” step.

After upgrading Hands:

- Re-run `hands browser extension install` to refresh the installed files under your Hands state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Hands ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Hands to control.
...
- `ON`: attached; Hands can drive that tab.
...
Debugging: `hands sandbox explain`
...
`hands browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `hands browser extension install` first to copy the extension to a stable location under your Hands state directory.
...
- **This is not isolated** like the dedicated hands-managed profile.
