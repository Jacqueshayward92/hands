---
summary: "Agent tool surface for Hands (browser, canvas, nodes, message, cron) replacing legacy `hands-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `hands-*` skills
title: "Tools"
---

# Tools (Hands)

Hands exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `hands-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `hands.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Hands logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:hands`: all built-in Hands tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`hands node run`).
...
- Requires a Brave API key (recommended: `hands configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Hands-managed browser.
...
- Quick smoke: `hands nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `hands gateway` restart in-place)
