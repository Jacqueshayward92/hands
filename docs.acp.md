...
This document describes how the Hands ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`hands acp` exposes an ACP agent over stdio and forwards prompts to a running
Hands Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Hands Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `hands acp` over stdio.

Example config:

```bash
hands config set gateway.remote.url wss://gateway-host:18789
hands config set gateway.remote.token <token>
```

Example run:

```bash
hands acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
hands acp --session agent:main:main
hands acp --session agent:design:main
hands acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Hands ACP” to start a thread.

## Execution Model

- ACP client spawns `hands acp` and speaks ACP messages over stdio.
...
```bash
hands acp --session agent:main:main
hands acp --session-label "support inbox"
hands acp --reset-session
```
...
`hands acp` resolves the Gateway URL and auth from CLI flags or config:
